...
export const deleteCard = async (
  ...
  try {
    ...
    const ids = idsString.split('_');

    // Get the list with the first-deleted-card
    const firstDeletedCard = await CardModel.findById(ids[0]);
    if (!firstDeletedCard) {
      return res.status(404).json({ error: "first id is not valid" });
    }
    const list = await ListModel.findById(firstDeletedCard.list_id);
    if (!list) {
      return res.status(404).json({ error: "list_id is not valid" });
    }
    ...
    list.cards = list.cards.filter((cardId) => !ids.includes(cardId.toString()));
    await list.save();
