...
export const PostsProvider = ({ children }: PropsWithChildren) => {
  ...
  const updatePost = async (index: number, title: string, content: string) => {
    if (posts === null) return;
    try {
      const postId = posts[index]._id; // Assuming each post has an '_id' property
      await PostService.update(postId, { title, content, author: 'author' });
  ...
  const votePost = async (
    ...
    if (vote === 'upvote') {
      PostService.upvote(posts[index]._id, userId)
    } else if (vote === 'downvote') {
      PostService.downvote(posts[index]._id, userId)
    }
  };
